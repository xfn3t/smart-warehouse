...
public class DashboardPublisher {
    ...
    public void publishHeartbeat() {
        try {
            List<Robot> robots = robotService.findAllWithWarehouse();
            for (Robot robot : robots) {
                try {
                    if (robot.isDeleted()) continue;
                    if (robot.getWarehouse() == null) {
                        log.warn(
                            "Skipping robot {} without warehouse",
                            robot.getCode()
                        );
                        continue;
                    }
                    ...
                    log.warn(
                        "Failed to publish heartbeat for robot {}: {}",
                        robot.getCode(),
                        e.getMessage()
                    );
                }
    ...
    private void publishForRobot(Robot robot) throws Exception {
        ...
        List<String> raw = redisTemplate
            .opsForList()
            .range(redisKey, -robotProperties.getRecentScansLimit(), -1);
