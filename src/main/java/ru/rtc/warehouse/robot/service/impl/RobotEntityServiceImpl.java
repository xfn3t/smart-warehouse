...
public class RobotEntityServiceImpl implements RobotEntityService {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<Robot> findAllWithWarehouse() {
        return robotRepository.findAllWithWarehouse();
    }
    ...
    public Integer getTotalRobotsCount(Long warehouseId) {
        ...
        try {
            Integer count = robotRepository.countByWarehouseIdAndIsDeletedFalse(
                warehouseId
            );
            log.info(
                "Total robots count for warehouse {}: {}",
                warehouseId,
                count
            );
            return count;
            ...
            log.error(
                "Error getting robots count for warehouse: {}",
                warehouseId,
                e
            );
    ...
    public List<Robot> findByWarehouseId(Long warehouseId) {
        ...
        try {
            return robotRepository.findByWarehouseIdAndIsDeletedFalse(
                warehouseId
            );
        } catch (Exception e) {
            log.error("Error finding robots for warehouse: {}", warehouseId, e);
            throw new RuntimeException(
                "Failed to retrieve robots by warehouse",
                e
            );
        }
    }
