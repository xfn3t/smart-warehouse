    ...
    implements ProductLastInventoryService
{
    ...
    public ProductLastInventoryPageDTO getLastInventoryByWarehouse(
        ...
        log.info(
            "Getting last inventory records for warehouse: {}",
            warehouseCode
        );
        ...
        // Применяем фильтры
        List<InventoryHistory> filtered = applyFilters(
            allLatest,
            searchRequest
        );
        ...
        // Сортируем
        List<InventoryHistory> sorted = applySorting(
            filtered,
            pageable.getSort()
        );
    ...
    private List<InventoryHistory> applyFilters(
    ...
    private List<InventoryHistory> applySorting(
        List<InventoryHistory> records,
        Sort sort
    ) {
        if (sort.isUnsorted()) {
            ...
            return records
                ...
                .sorted((a, b) -> {
                    if (
                        a.getScannedAt() == null && b.getScannedAt() == null
                    ) return 0;
        ...
        switch (field) {
            case "productCode":
                return (
                    multiplier *
                    compareStrings(
                        a.getProduct().getSkuCode(),
                        b.getProduct().getSkuCode()
                    )
                );
            case "productName":
                return (
            ...
            case "lastScannedAt":
                return (
                    multiplier *
                    compareDates(a.getScannedAt(), b.getScannedAt())
                );
            case "statusCode":
                return (
                    multiplier *
