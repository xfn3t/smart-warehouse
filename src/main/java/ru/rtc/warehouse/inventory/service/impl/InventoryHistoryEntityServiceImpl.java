...
public class InventoryHistoryEntityServiceImpl
    implements InventoryHistoryEntityService
{
    ...
    public InventoryHistory findById(Long id) {
        return inventoryHistoryRepository
            .findById(id)
            .orElseThrow(() ->
                new NotFoundException("Inventory history not found")
            );
    }
    ...
    public InventoryHistory findByProductSKU(String sku, String warehouseCode) {
        return inventoryHistoryRepository
            .findByProductSKU(sku, warehouseCode)
            .orElseThrow(() ->
                new NotFoundException("Inventory history not found")
            );
    }
    ...
    @Override
    public List<InventoryHistory> findAllByWarehouseCodeAndProductCode(
        String warehouseCode,
        String productCode
    ) {
        return inventoryHistoryRepository.findAllByWarehouseCodeAndProductSkuCode(
            warehouseCode,
            productCode
        );
    }
    ...
    @Override
    public long countByWarehouseAndScannedAtBetween(
        Warehouse warehouse,
        LocalDateTime todayStart,
        LocalDateTime todayEnd
    ) {
        return inventoryHistoryRepository.countByWarehouseAndScannedAtBetween(
            warehouse,
            todayStart,
            todayEnd
        );
    ...
    @Override
    public long countByWarehouseAndStatusAndScannedAtAfter(
        Warehouse warehouse,
        InventoryHistoryStatus.InventoryHistoryStatusCode inventoryHistoryStatusCode,
        LocalDateTime last24Hours
    ) {
        return inventoryHistoryRepository.countByWarehouseAndStatusAndScannedAtAfter(
            warehouse,
            inventoryHistoryStatusCode,
            last24Hours
        );
    }

    @Override
    public boolean existsByZoneAndRowAndShelfAndWarehouseAndScannedAtAfter(
        Integer zone,
        Integer row,
        Integer shelf,
        Warehouse warehouse,
        ...
        return inventoryHistoryRepository.existsByZoneAndRowAndShelfAndWarehouseAndScannedAtAfter(
            zone,
            row,
            shelf,
            warehouse,
    ...
    @Override
    public InventoryHistory findLatestBySkuAndWarehouseCode(
        String sku,
        String warehouseCode
    ) {
        return inventoryHistoryRepository
            .findLatestBySkuAndWarehouseCode(sku, warehouseCode)
            .orElseThrow(() ->
                new NotFoundException("Inventory history not found")
            );
    }
    ...
    public InventoryHistory findLatestByProductId(Long productId) {
        ...
        try {
            ...
            if (history.isPresent()) {
                log.debug(
                    "Found inventory history for product {}: quantity={}, expected={}",
                    productId,
                    history.get().getQuantity(),
                    history.get().getExpectedQuantity()
                );
                return history.get();
            } else {
                log.debug(
                    "No inventory history found for product: {}",
                    productId
                );
                return null;
            }
    ...
    @Transactional(readOnly = true)
    public List<Object[]> findScanCountsByRobotAndPeriod(
        LocalDateTime startDate,
        LocalDateTime endDate
    ) {
        log.info(
            "Finding scan counts by robot for period: {} to {}",
            startDate,
            endDate
        );
        ...
        try {
            List<Object[]> results =
                inventoryHistoryRepository.findScanCountsByRobotAndPeriod(
                    startDate,
                    endDate
                );
            ...
            log.error(
                "Error finding scan counts by robot for period: {} to {}",
                startDate,
                endDate,
                e
            );
    ...
    public List<InventoryHistory> findByProductId(Long productId) {
        ...
        try {
            return inventoryHistoryRepository
                ...
                .stream()
                .filter(history ->
                    history.getProduct().getId().equals(productId)
                )
                .filter(history -> !history.isDeleted())
                .collect(java.util.stream.Collectors.toList());
        } catch (Exception e) {
            log.error(
                "Error finding inventory history for product: {}",
                productId,
                e
            );
            throw new RuntimeException(
                "Failed to retrieve inventory history by product",
                e
            );
        }
    }
