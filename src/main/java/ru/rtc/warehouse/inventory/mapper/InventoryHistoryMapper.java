...
public abstract class InventoryHistoryMapper {
    ...
    public abstract List<InventoryHistoryDTO> toDtoList(
    ...
    @Mappings({
        @Mapping(
            target = "robot",
            source = "robotCode",
            qualifiedByName = "resolveRobot"
        ),
        @Mapping(
            target = "product",
            source = "skuCode",
            qualifiedByName = "resolveProductBySku"
        ),
        @Mapping(
            target = "status",
            source = "status",
            qualifiedByName = "resolveStatus"
        ),
    })
    ...
    public abstract InventoryHistory toEntity(
        InventoryHistoryCreateRequest dto
    );
    ...
    public abstract InventoryHistory toEntity(
        InventoryHistoryUpdateRequest dto
    );

    public abstract List<InventoryHistory> toEntityList(
        List<InventoryHistoryDTO> dtos
    );
    ...
    protected Product resolveProductBySku(String sku) {
        return (sku == null || sku.isBlank())
            ? null
            : productService.findByCode(sku);
    }

    @Named("resolveStatus")
    protected InventoryHistoryStatus resolveStatus(
        InventoryHistoryStatus.InventoryHistoryStatusCode code
    ) {
        if (code == null) return null;
        return statusRepo
            .findByCode(code)
            .orElseThrow(() ->
                new IllegalArgumentException("Status not found: " + code)
            );
    }
}
