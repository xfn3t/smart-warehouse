...
public class DashboardServiceImpl implements DashboardService {
    ...
    public List<RobotDashboardDTO> getAllRobotsForDashboard() {
        ...
        List<Robot> robots = robotEntityService.findAllWithWarehouse();
        return robots
            .stream()
    ...
    public WarehouseStatsDTO getWarehouseStats(String warehouseCode) {
        ...
        List<Robot> warehouseRobots = robotEntityService.findAllByWarehouseCode(
            warehouseCode
        );
        ...
        WarehouseMetricsDTO metrics = calculateWarehouseMetrics(
            warehouse,
            warehouseRobots
        );
    ...
    public WarehouseLocationsDTO getWarehouseLocations(String warehouseCode) {
        ...
        List<LocationMetricsDTO> locationMetrics = distinctLocations
            .stream()
            .map(row -> {
                Integer zone = (Integer) row[0];
                Integer r = (Integer) row[1];
                Integer shelf = (Integer) row[2];
                return locationMetricsService.computeFor(
                    warehouse,
                    zone,
                    r,
                    shelf
    ...
    public WarehouseRobotsDTO getWarehouseRobots(String warehouseCode) {
        ...
        List<Robot> robots = robotEntityService.findAllByWarehouseCode(
            warehouseCode
        );
        ...
        List<WarehouseRobotDTO> robotDTOs = robots
            .stream()
            .filter(robot -> !robot.isDeleted())
    ...
    private RobotDashboardDTO convertToRobotDashboardDTO(Robot robot) {
        ...
        dto.setZone(robot.getCurrentZone());
        dto.setRow(robot.getCurrentRow());
        dto.setShelf(robot.getCurrentShelf());
    ...
    private WarehouseMetricsDTO calculateWarehouseMetrics(
        Warehouse warehouse,
        List<Robot> robots
    ) {
        WarehouseMetricsDTO metrics = new WarehouseMetricsDTO();

        List<Robot> activeRobots = robots
            .stream()
            .filter(robot -> !robot.isDeleted())
            .collect(Collectors.toList());
        ...
        long workingRobots = activeRobots
            .stream()
            .filter(robot ->
                "WORKING".equals(robot.getStatus().getCode().name())
            )
            .count();
        metrics.setActive_robots((int) workingRobots);

        long chargingRobots = activeRobots
            .stream()
            .filter(robot ->
                "CHARGING".equals(robot.getStatus().getCode().name())
            )
            .count();
        metrics.setCharging_robots((int) chargingRobots);

        long errorRobots = activeRobots
            .stream()
        ...
        Map<String, Integer> statusDistribution = calculateStatusDistribution(
            activeRobots
        );
        ...
        long totalScansToday =
            inventoryHistoryEntityService.countByWarehouseAndScannedAtBetween(
                warehouse,
                todayStart,
                todayEnd
            );
    ...
    private BatteryLevelsDTO calculateBatteryLevels(List<Robot> robots) {
        ...
        OptionalDouble avgBattery = robots
            .stream()
            .mapToInt(Robot::getBatteryLevel)
            .average();
        batteryLevels.setAverage(
            avgBattery.isPresent() ? (int) avgBattery.getAsDouble() : 0
        );

        OptionalInt minBattery = robots
            .stream()
            .mapToInt(Robot::getBatteryLevel)
            .min();
        batteryLevels.setLowest(
            minBattery.isPresent() ? minBattery.getAsInt() : 0
        );

        OptionalInt maxBattery = robots
            .stream()
            .mapToInt(Robot::getBatteryLevel)
            .max();
        batteryLevels.setHighest(
            maxBattery.isPresent() ? maxBattery.getAsInt() : 0
        );

        return batteryLevels;
    }

    private Map<String, Integer> calculateStatusDistribution(
        List<Robot> robots
    ) {
        return robots
            .stream()
    ...
    private AlertStatsDTO calculateAlertStats(Warehouse warehouse) {
        ...
        long lowStockAlerts =
            inventoryHistoryEntityService.countByWarehouseAndStatusAndScannedAtAfter(
                warehouse,
    ...
    private boolean hasRecentScans(
        Integer zone,
        Integer row,
        Integer shelf,
        Warehouse warehouse,
        ...
        try {
            return inventoryHistoryEntityService.existsByZoneAndRowAndShelfAndWarehouseAndScannedAtAfter(
                zone,
                row,
                shelf,
                warehouse,
            ...
            log.debug(
                "Error checking recent scans for location {}-{}-{}: {}",
                zone,
                row,
                shelf,
                e.getMessage()
    ...
    private List<ScanDTO> getRecentScansFromRedis(String robotCode, int limit) {
        try {
            ...
            List<String> rawScans = redisTemplate
                .opsForList()
                .range(redisKey, -limit, -1);
            ...
            return rawScans
                ...
                .map(scanJson -> {
                    try {
                        Map<String, Object> scanMap = objectMapper.readValue(
                            scanJson,
                            Map.class
                        );
                        ...
                        log.warn(
                            "Failed to parse scan JSON for robot {}: {}",
                            robotCode,
                            e.getMessage()
                        );
                        return null;
            ...
            log.warn(
                "Failed to get recent scans from Redis for robot {}: {}",
                robotCode,
                e.getMessage()
            );
            return new ArrayList<>();
