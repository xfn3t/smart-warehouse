...
public class RobotDataIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        // prepare DB: insert minimal rows (warehouse, robot_status, product, robot)
        // Use explicit IDs to simplify assertions
        jdbcTemplate.update(
            "INSERT INTO warehouses (id, code, zone_max_size, row_max_size, shelf_max_size, name, is_deleted) " +
                "VALUES (100, 'WH-TEST', 5, 10, 10, 'Test Warehouse', FALSE) ON CONFLICT DO NOTHING"
        );

        jdbcTemplate.update(
            "INSERT INTO robot_status (id, code, is_deleted) VALUES (10, 'IDLE', FALSE) ON CONFLICT DO NOTHING"
        );

        // location table removed — zone/row/shelf stored directly on robots and inventory_history

        jdbcTemplate.update(
            "INSERT INTO products (id, sku_code, name, warehouse_id, is_deleted, min_stock, optimal_stock) " +
                "VALUES (300, 'TEL-4567', 'Тестовый товар', 100, FALSE, 1, 100) ON CONFLICT DO NOTHING"
        );

        jdbcTemplate.update(
            "INSERT INTO robots (id, robot_code, warehouse_id, status_id, battery_level, current_zone, current_row, current_shelf, is_deleted) " +
                "VALUES (400, 'RB-0001', 100, 10, 50, 1, 1, 1, FALSE) ON CONFLICT DO NOTHING"
        );

        // ensure no leftover in recent_scans
        String recentKey = String.format(
        ...
        try {
            jdbcTemplate
                ...
                .getConnection()
                .createStatement()
                .execute("SELECT 1"); // noop, just to ensure connection
        } catch (Exception e) {
            // ignore
        }
        ...
        lettuceClient = RedisClient.create(
            RedisURI.create(redisHost, redisPort)
        );
    ...
    void tearDown() {
        ...
        jdbcTemplate.update(
            "DELETE FROM inventory_history WHERE warehouse_id = 100"
        );
        jdbcTemplate.update("DELETE FROM robot_tokens WHERE robot_id = 400");
    }

    @Test
    void testProcessRobotData_writesInventoryAndPublishesToRedis()
        throws Exception {
        ...
        Object reqObj = objectMapper.convertValue(
            requestMap,
            Class.forName(
                "ru.rtc.warehouse.robot.controller.dto.request.RobotDataRequest"
            )
        );

        // call service (this will update DB, Redis)
        Object resp = robotDataService
            .getClass()
        ...
        Awaitility.await()
            ...
            .untilAsserted(() -> {
                ...
                try (var conn = lettuceClient.connect()) {
                    var sync = conn.sync();
                    java.util.List<String> list = sync.lrange(
                        recentKey,
                        -10,
                        -1
                    );
                    ...
                    assertTrue(
                        !list.isEmpty(),
                        "recent scans list in redis should contain items"
                    );
                }
            });
        ...
        Awaitility.await()
            ...
            .untilAsserted(() -> {
                ...
                assertTrue(
                    m.contains("\"type\""),
                    "payload should be JSON with 'type'"
                );
                assertTrue(
                    m.contains("robot_update") || m.contains("robot_status"),
                    "published message should be robot_update or robot_status"
                );
            });
