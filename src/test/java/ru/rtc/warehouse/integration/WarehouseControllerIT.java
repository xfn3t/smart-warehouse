...
class WarehouseControllerIT {
    ...
    void seed() {
        ...
        jdbc.update(
            """
                TRUNCATE TABLE user_warehouses, warehouses, users, roles
                RESTART IDENTITY CASCADE
            """
        );

        jdbc.update(
            "INSERT INTO roles(id, code, is_deleted) VALUES (?, ?, false)",
            ROLE_ID,
            ROLE_CODE
        );
        ...
        Integer whCount = jdbc.queryForObject(
            "SELECT COUNT(*) FROM warehouses",
            Integer.class
        );
        Integer uwCount = jdbc.queryForObject(
            "SELECT COUNT(*) FROM user_warehouses",
            Integer.class
        );
    ...
    @Order(1)
    @DisplayName(
        "GET /api/warehouse — возвращает только склады пользователя из тестовой БД"
    )
        ...
        System.out.println(
            "\n[Ответ контроллера /api/warehouse]\n" +
                om
                    ...
                    .writeValueAsString(om.readTree(raw))
        );
    ...
    @Test
    @Order(2)
    @DisplayName("POST /api/warehouse — создаёт склад и линкует к пользователю")
    void createWarehouse_persists_and_links_ok() throws Exception {
        step(
            "Шаг 2: отправляем POST /api/warehouse (ADMIN) и ждём 201 Created"
        );
        ...
        assertThat(linkCount)
            .as("Созданный склад должен быть привязан к пользователю")
            .isEqualTo(1);
        ok(
            "Связь user_warehouses создана (user_id=" +
                U_ID +
                ", warehouse_id=" +
                newId +
                ")"
        ...
        step("ИТОГО: POST создал склад и связал с пользователем — всё ок ✅");
    }
}
