...
@DisplayName("InventoryHistoryQueryService IT (реальная БД, профиль test)")
class InventoryHistoryServiceIT {
    ...
    @Autowired
    InventoryHistoryQueryService service;

    private static void step(String t) {
        System.out.println(
            "\n" + "═".repeat(80) + "\n▶ " + t + "\n" + "═".repeat(80)
        );
    }
    ...
    @BeforeEach
    void seed() {
        step("DB SEED: TRUNCATE + справочники + тестовые данные");
        ...
        // ── справочник статусов истории
        jdbc.update(
            "INSERT INTO inventory_status(code, is_deleted) VALUES (?, false)",
            "OK"
        );
        jdbc.update(
            "INSERT INTO inventory_status(code, is_deleted) VALUES (?, false)",
        ...
        ok("Справочники inventory_status вставлены");
        ...
        Long stCrit = id(
            "SELECT id FROM inventory_status WHERE code='CRITICAL'"
        );

        // ── роботы
        jdbc.update(
            "INSERT INTO robots(warehouse_id, robot_code, battery_level, is_deleted) VALUES (?,?,100,false)",
            w1,
            "RB-1"
        );
        jdbc.update(
        ...
        // ── история (zone, row, shelf directly in inventory_history)
    ...
    @Test
    @DisplayName(
        "Скоуп по складу + период + фильтр по статусам + дефолтная сортировка"
    )
    void scopes_period_status_defaultSort() {
        ...
        rq.setStatuses(
            List.of(InventoryHistoryStatus.InventoryHistoryStatusCode.CRITICAL)
        );

        Pageable pg = PageRequest.of(0, 20, Sort.unsorted());
        ...
        assertThat(page.getItems()).isSortedAccordingTo((a, b) ->
            b.getScannedAt().compareTo(a.getScannedAt())
        );
    ...
    @Test
    @DisplayName(
        "Ручная пагинация: стабильный размер страницы и корректные номера"
    )
    void pagination_manualSlice() {
        ...
        for (int i = 0; i < 25; i++) {
            insertHistory(
                ...
                60,
                base.plusHours(i),
                base.plusHours(i).plusMinutes(1)
            );
        }
        ...
        var pageable = PageRequest.of(
            1,
            10,
            Sort.by(Sort.Order.desc("scannedAt"))
        );
        ...
        jdbc.update(
            """
                INSERT INTO inventory_history(
                    warehouse_id, zone, "row", shelf, robot_id, product_id, status_id,
    ...
    private Long upsertRobot(Long wId, String code) {
        Integer exists = jdbc.queryForObject(
            "SELECT COUNT(*) FROM robots WHERE robot_code=?",
            Integer.class,
            code
        );
        if (exists != null && exists > 0) return id(
            "SELECT id FROM robots WHERE robot_code=?",
            code
        );
        jdbc.update(
            "INSERT INTO robots(warehouse_id, robot_code, battery_level, is_deleted) VALUES (?,?,100,false)",
    ...
    private Long upsertProduct(
        ...
        Integer exists = jdbc.queryForObject(
            "SELECT COUNT(*) FROM products WHERE sku_code=?",
            Integer.class,
            sku
        );
        if (exists != null && exists > 0) return id(
            "SELECT id FROM products WHERE sku_code=?",
            sku
        );
        jdbc.update(
            """
